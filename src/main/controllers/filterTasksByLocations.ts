...
export function filterTasksByLocations(req: Request, res: Response): void {
  ...
  req.session.addLocations = model.getAddLocations().filter(x => !locations.includes(x.name));
  req.session.removeLocations = model.getAddLocations().filter(x => locations.includes(x.name));
